...
from declat.declat import get_max_support_value
from declat.readDataset import read_data_set


def measure_time(path_to_dataset: str, min_support: int):
    print('Loading dataset')
    ds = read_data_set(path_to_dataset)
    print('Finding support values in the database')
    dr = DECLATRunner()
    max_support_value = get_max_support_value(dr.run(ds, min_support), len(ds.transactions))
